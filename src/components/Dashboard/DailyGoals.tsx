...
const date = ["Sun", "Sat", "Fri", "Thu", "Wed", "Tue", "Mon"];
...
const Goal = styled.div<GoalProps>`
	...
	background-color: ${({ goal, isDark }) => {
		if (goal === -1) return isDark ? "#d1d1d11a" : "#ECEDED";
		if (goal < 2) return "#F3EBFF";
		else if (goal < 4) return "#CFAEFE";
		else if (goal < 6) return "#CFAEFE";
		else return "#8734FD";
	}};
`;
...
export function GoalsContainer({
	...
	const getDailyTask = async () => {
		try {
			...
			const res = await axios.get(
				...
				}/daily-tasks?date=${currentDate.getFullYear()}${String(
			...
			if (data.length < 35) {
				...
				data = data.concat(
					Array(diff).fill({ date: "", task_count: -1 })
				);
			}
			setGoals(data);
		} catch (error) {
	...
	return (
		<div className={styles.goalsContainer} style={{ color: "#000" }}>
			{rotated.map((g, i) => (
				<Goal key={i} isDark={isDark} goal={g.task_count} />
			))}
		</div>
	);
}
