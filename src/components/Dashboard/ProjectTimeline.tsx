...
const endDay = [
	...
	new Date(
		today.getFullYear(),
		today.getMonth(),
		today.getDate() + diffToSun
	),
];
for (let i = 0; i < 3; i++) {
	console.log(startDay[i], endDay[i]);
}
const percent = [0.274, 100 / endDay[1].getDate(), 14.28];
...
export function Project({
	...
	const daysLeft =
		(start.getTime() - startDay[timeUnit].getTime()) / msPerDay;
	const widthBool = end > endDay[timeUnit];
	const leftBool = start < startDay[timeUnit];
	const containerWidth = widthBool
		? 100
		: (daysWidth + 1) * percent[timeUnit];
...
export default function ProjectTimeline() {
	...
	const getAll = async () => {
		try {
			const res = await axios.get(
				`${import.meta.env.VITE_BASE_URL}/projects/timeline`,
				{
					headers: {
	...
	useEffect(() => {
		...
		for (let i = 0; i < allProjects.length; i++) {
			if (
				isDateInRange(new Date(allProjects[i].start_date)) ||
				isDateInRange(new Date(allProjects[i].end_date))
			)
						...
						return (
							<Line key={i} isDark={isDark}>
								<Project
									...
									timeUnit={timeUnit}
									background={backColor[random].background}
									bar={backColor[random].bar}
								/>
							</Line>
						);
					})}
