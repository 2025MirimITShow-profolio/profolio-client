...
import LoginContainer, {InputBox, NextBtn, AuthenticationInput} from '../components/LonginContainer'
...
export function Id({setStep, email, setEmail}:SetStepProps & {
    ...
    return (
        <>
            ...
            <NextBtn isDark={isDark} disabled={email===''?true:false} onClick={() => setStep(1)}>
                다음
            </NextBtn>
            <div className={styles.noIdContainer}>
                <span>아이디를 가지고 있나요?</span>
                <span className={styles.signupBtn} onClick={() => navigate('/login')}>Log in</span>
            </div>
        </>
    )
}
...
export function Authentication({setStep}:SetStepProps) {
    ...
    return (
        <>
            ...
            <span className={styles.subText}>이메일로 인증 코드를 보냈습니다</span>
            ...
            <NextBtn isDark={isDark} disabled={!authentication} onClick={() => setStep(2)}>
                다음
            </NextBtn>
            <div className={styles.noIdContainer}>
...
export function Password({setStep, password, setPassword}:SetStepProps & {
    ...
    return (
        <>
            ...
            <span className={styles.subText}>로그인 시 사용할 비밀번호를 입력해주세요</span>
            <InputBox isDark={isDark}>
                <input
...
export function Profile({setStep, profileImg, setProfileImg, name, setName, job, setJob, birth, setBirth}:SetStepProps & {
    ...
    return (
        <>
            <p className={styles.text} style={{textAlign: 'center'}}>프로필을 만들어주세요</p>
            <div className={styles.profileContainer}>
                ...
                {showProfile && (
                    <div className={styles.profileImgContainer} style={{backgroundColor: isDark?'#2B2B37':'#FFF'}}>
                        {profileNumber.map((i) => (
                            <div
                                key={i}
                                className={styles.profileImg}
                                style={i===profileImg?{border: '2px solid #8734FD'}:{}}
                        ...
                        <span className={styles.selectBtn} onClick={() => setShowProfile(false)}>선택</span>
                    </div>
                )}
            </div>
